...
void send_motor_dm_cmd(uint16_t motor_id, float kp, float kd, float pos, float spd, float tor, uint8_t* Data, uint32_t* canID)
{
    ...
    *canID = motor_id;
    ...
    else if (kp < DM_KP_MIN){kp = DM_KP_MIN;}
    if (kd > DM_KD_MAX){kd = DM_KD_MAX;}
    else if (kd < DM_KD_MIN){kd = DM_KD_MIN;}
    if (pos > DM_P_MAX){pos = DM_P_MAX;}
    else if (pos < DM_P_MIN) {pos = DM_P_MIN;}
    if (spd > DM_V_MAX){spd = DM_V_MAX;}
    else if (spd < DM_V_MIN){spd = DM_V_MIN;}
    if (tor > DM_T_MAX){tor = DM_T_MAX;}
    else if (tor < DM_T_MIN){tor = DM_T_MIN;}
...
void Gripper_can_data_repack(uint32_t msgID, uint8_t* Data)
{
    ...
    if(rv_motor_msg[8].init == 0){
        rv_motor_msg[8].init_angle=rv_motor_msg[8].gripper_pos;
        rv_motor_msg[8].init = 1;
    }
    ...
    if (rv_motor_msg[8].gripper_pos - rv_motor_msg[8].gripper_last_pos > 32768)
        ...
        rv_motor_msg[8].round_cnt++;
    rv_motor_msg[8].gripper_totalangle = rv_motor_msg[8].round_cnt * 65536 + rv_motor_msg[8].gripper_pos-rv_motor_msg[8].init_angle;
